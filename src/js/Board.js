...
export default class Board {
  ...
  registerEvents() {
    ...
    addNewCardButtonElement.forEach((element) => element.addEventListener('click', (event) => this.onClickAddNewCard(event)));
  ...
  onMouseDown(event) {
    ...
    this.showGhostElement(height, width, currentElement);
  ...
  onMouseMove(event) {
    ...
    const taskColumn = document.elementsFromPoint(event.clientX, event.clientY)[2].closest('.tasks-container');
    const bottomElement = document.elementsFromPoint(event.clientX, event.clientY)[2].closest('li');
    ...
    if (!taskColumn.querySelectorAll('.ul').length) {
      const columnTasks = taskColumn.querySelector('.task-items');
      columnTasks.insertAdjacentElement('beforeend', this.ghostEl);
    }

    if (event.pageY < bottomElement.offsetTop + (bottomElement.offsetHeight / 2)) {
      bottomElement.insertAdjacentElement('beforebegin', this.ghostEl);
    } else {
      bottomElement.insertAdjacentElement('afterend', this.ghostEl);
    }
  }
  ...
  onMouseUp(event) {
    ...
    if (event.target.closest('.tasks-container')) {
      this.ghostEl.replaceWith(this.currentElement);
    }
    this.ghostEl.remove();
  }
  ...
  hiddenDeleteButton(event) {
    const buttonElement = event.target.querySelector('button');
    buttonElement.classList.add('hidden');
  }

  showGhostElement(height, width, currentElement) {
    const ghostEl = document.createElement('div');
    ghostEl.classList.add('card-ghost-element');
    currentElement.insertAdjacentElement('afterend', ghostEl);
